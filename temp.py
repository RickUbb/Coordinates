...
def get_coordinates():
    ...
    """
    data = request.get_json()  # Obtener los datos del cuerpo de la solicitud en formato JSONz
    # Normalizar los datos de entrada (país, provincia, ciudad)
    # province_direction = f"{country}, {province}" if country != "NA" else province
    subnivel_1 = normalize(
        data['country']) if 'country' in data and data['country'] else "NA"

    # País
    subnivel_3 = normalize(
        # Provincia
        data['province']) if 'province' in data and data['province'] else "NA"
    subnivel_4 = normalize(
        data['city']) if 'city' in data and data['city'] else "NA"  # Ciudad
