...
extraer información de regiones, realizar consultas a una API externa para obtener coordenadas 
(latitud y longitud) basadas en dicha información, y luego actualizar los documentos en MongoDB 
    ...
    try:
        if " | " in region:
            ...
            if region_type in REGION_MAP:
                ...
                # Ejemplo: si es "Province", lo mapeamos como {"province": "Nombre de la región"}.
...
def process_kafka_message(document_id, type_value):
    """
    Procesa un mensaje de Kafka que contiene el ID del documento y el tipo de documento.

    Args:
        document_id (str): ID del documento en MongoDB.
        type_value (str): Tipo de documento que determina la colección en MongoDB.

    Raises:
        Exception: Si ocurre un error en el procesamiento del mensaje.
    """
    try:
        collection_name = get_collection_from_type(type_value)
        document = fetch_document_by_id(collection_name, document_id)

        if not document:
            raise ValueError(f"Documento con _id={document_id} no encontrado.")

        for region_obj in document.get('region_distri', []):
            region = region_obj.get('region', None)
